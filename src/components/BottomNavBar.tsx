...
export default function BottomNavBar({ onOpenUpload }: { onOpenUpload?: () => void }) {
  ...
  const getTabClass = (path: string) => {
    ...
    return `flex flex-col items-center justify-center w-14 transition-all duration-300 ${
      isActive 
        ? "text-[#034AA6] scale-110 font-bold drop-shadow-md" 
        : "text-[#3C74A6] hover:text-[#034AA6] hover:scale-110 active:scale-90"
    }`;
  };

  return (
    <nav className="fixed bottom-0 w-full z-40 bg-[#F2B950] border-t border-black/10 pb-safe rounded-t-3xl shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
      <div className="flex justify-between items-center h-20 px-6 max-w-[768px] mx-auto relative">
        ...
        <div className="relative -top-8">
          <button 
            ...
            className="w-14 h-14 bg-[#3C74A6] text-white rounded-full shadow-lg flex items-center justify-center active:scale-90 hover:scale-110 transition-all duration-300 border-4 border-[#F2B950] cursor-pointer"
          >
